<template>
  <div >
    ...
    <v-sheet>
      <v-card tile flat class="pt-0 px-2">
        <v-textarea
      ...
      <v-card tile flat class="px-6 py-0 d-flex justify-start">
        <!--输入框下部表情能按钮-->
...
export default {
  ...
  methods: {
    ...
    selectEmoji(emoji) {
      ...
      this.sendStatus = true;
      this.$store.dispatch(
        "Post/commentDialogCommentAction",
        this.tweet + emoji.data
      );
    },
    // 预览路径
    ...
    updateTweet: function (e) {
      this.$store.dispatch("Post/commentDialogCommentAction", e);
    },
    ...
    postUploadFile: function (tokenCount) {
      ...
      for (let j = 0; j < this.uploadFilesList.length; j++) {
        //新增上传对象
        new tus.Upload(this.uploadFilesList[j], this.getOptions(j)).start();
      }
        ...
        .then((response) => {
          if (response.data.Code === 0) {
            ...
            if (response.data["Body"].T) {
              ...
              User.UserD = userinfo.DisplayName;
              User.UserA = userinfo.Avatar96;

              // 组装json
              this.$store.dispatch("Post/msgJSONAction", {
                Type: "Tw",
                Tw: this.userState.tweet,
              });
              this.$store.dispatch("Post/msgJSONAction", {
                Type: "T",
              ...
              console.log(
                "组装多媒体消息体：" +
                  JSON.stringify(this.$store.getters["Post/msgJSONState"])
              );

              this.$store.dispatch("Post/messagesAction", {
                type: "unshift",
                message: this.$store.getters["Post/msgJSONState"],
              });
            }
            ...
            this.$store.dispatch("Post/msgJSONAction", { Type: "P", P: null });
            this.$store.dispatch("Post/msgJSONAction", {
              Type: "Url",
              Url: "",
            });
