...
<v-sheet tile color="grey lighten-4">
  ...
  <v-card tile flat class="my-2" >
    ...
    <!--START LOGO-->
    <img class="my-2 mx-4" src='../../assets/img/VOG_LOGO.png'  height="36"  alt="logo" />
    <!--END LOGO-->

      <v-list  tile light nav subheader class="">
        <v-list-item-group
            v-model="menuStatus"
            mandatory
            color="info"
        ...
        <v-list-item  link @click.stop="homePage">
          ...
          <v-list-item-content>
            <v-list-item-title class="text-h6 font-weight-black" >Home</v-list-item-title>
          </v-list-item-content>
        ...
        <v-list-item link @click.stop="notifyPage">
          <v-list-item-icon>
            <v-badge
                color="light-blue accent-3"
                overlap
                :content="$store.getters['Post/notifyMsgsState'].length"
                :value="$store.getters['Post/notifyMsgsState'].length > 0"
            >
          ...
          <v-list-item-content>
            <v-list-item-title class="text-h6 font-weight-black"  >Notifications</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <!--END 菜单Notifications-->
...
export default {
  ...
  methods: {
    ...
    //获取群组内活跃的用户
    getGroupActiveUser: function (uid) {
      //清空之前的信息

      this.api.getGroupActiveUserAPI(uid, this.api.getAuthToken()).then(response => {
      if(response.data.Code === 0) {
        console.log("获取活跃用户成功:"+JSON.stringify(response.data))

        //存入store
        this.$store.dispatch('Post/actUsersAction',response.data['Body'])
    ...
    connectHomeSSE: function () {
      ...
      console.log('home domain url:' + url)
    ...
    connectGroupSSE: function() {
      ...
      sseGroup = this.$sse.create({
        url: curUrl + '/vgr1/stream/' + this.api.getAuthToken() + '/' + group_chatid,
        format: 'json',
        withCredentials: true,
        polyfill: true,
      });
